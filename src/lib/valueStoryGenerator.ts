...
LEGAL: "© ${new Date().getFullYear()} Microsoft Corporation. All rights reserved. This material is provided for informational purposes only. Microsoft makes no warranties, express or implied."`
  ...
  const coworkPrompts: CoworkPrompt[] = [
    {
      id: 'account-team-alignment',
      label: 'Account Team & Partner Brief',
      icon: '📋',
      description: 'Generate an HTML brief for the Microsoft STU, account team, and partners',
      prompt: `${fullContext}\n\nTASK: Create an internal alignment brief for the Microsoft account team (STU), specialist sellers, and partner ecosystem working on ${companyName}. Title: "${companyName} — Account Team Alignment & Opportunity Brief".\n\nThis is an INTERNAL Microsoft document — not customer-facing. It should give every team member the full picture to coordinate effectively.\n\nInclude:\n1. Account snapshot — ${companyName} (${data.companySize}, ${industryName}), key priorities, relationship context\n2. Frontier pillar alignment — which pillars are active, which priorities map to each, and the use cases selected\n3. Solution play mapping — for each use case, which Microsoft products/platforms are involved and which specialist sellers or solution areas should engage\n4. Evidence & proof points — the customer stories and ROI benchmarks that support each use case (for seller prep)\n5. Stakeholder map — who at ${companyName} owns each area, suggested Microsoft counterparts, and partner roles\n6. Partner opportunity — where systems integrators, ISVs, or consulting partners add value (implementation, industry expertise, managed services)\n7. Pursuit plan — recommended next steps: who does what, key milestones, and what needs validation before moving to implementation planning\n8. Open questions & risks — areas that still need customer confirmation, dependencies, and potential blockers\n\nTone: Direct, actionable, no fluff. This is a working document for the team.\n\nOUTPUT: Create this as a single HTML file (.html) with embedded CSS — clean, structured, easy to scan. Use Microsoft branding colors (#0078D4, #243A5E, #50E6FF). Designed to be shared via email or Teams. No external dependencies.`,
    },
    {
      id: 'exec-email',
    ...
    {
      id: 'business-case',
      label: 'Sales Proposal & Business Case',
      icon: '💰',
      description: 'Generate an HTML sales proposal to present to the customer',
