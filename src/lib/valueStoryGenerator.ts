...
export function generateValueStory(data: WizardData): ValueStory {
  ...
  const customerEvidenceBlock = uniqueStories.length > 0
    ? `\n\nCUSTOMER EVIDENCE (published Microsoft customer stories — include links in deliverables):\n${uniqueStories.map(s => `- ${s.company} (${s.title}): ${s.metric}${s.storyUrl ? ` | Source: ${s.storyUrl}` : ''}`).join('\n')}`
    : ''
...
LEGAL: "© ${new Date().getFullYear()} Microsoft Corporation. All rights reserved. This material is provided for informational purposes only. Microsoft makes no warranties, express or implied."`
  ...
  const coworkPrompts: CoworkPrompt[] = [
    ...
    },
    {
      ...
      prompt: `${fullContext}\n\nTASK: Draft an Outlook email to ${companyName}'s C-suite executive. Reference their top 2-3 priorities by name. Include one industry benchmark stat and one ROI projection from their selected inspirations. End with a clear CTA for a 30-minute strategy conversation. Under 250 words. Tone: strategic peer, not vendor pitch.\n\nOUTPUT: Create this as a draft email in Outlook, ready to review and send.`,
    },
    {
      ...
      icon: '🏆',
      phase: 'prep',
      description: `A customer-shareable brief mapping ${companyName}'s priorities to Microsoft's 5 key differentiators (Work IQ, Agentic, Agents Platform, Security, Multi-Model). Includes objection-handling talk tracks and public comparison links. Use when the customer is evaluating alternatives.`,
