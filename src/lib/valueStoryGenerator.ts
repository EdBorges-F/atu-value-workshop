...
LEGAL: "© ${new Date().getFullYear()} Microsoft Corporation. All rights reserved. This material is provided for informational purposes only. Microsoft makes no warranties, express or implied."`
  ...
  const coworkPrompts: CoworkPrompt[] = [
    ...
    },
    {
      ...
      icon: '🎯',
      description: 'Open the door — a short, strategic email to the C-suite referencing their priorities. Creates an Outlook draft ready to review and send. Best for first outreach or re-engagement.',
    ...
    },
    {
      ...
      icon: '📨',
      description: 'Confirm next steps — a post-meeting recap with priorities, aligned use cases, and action items. Creates an Outlook draft. Use after the customer conversation to lock in momentum.',
    ...
    },
    {
      ...
      icon: '📋',
      description: 'Hand off for execution — an internal Microsoft brief for STU, specialists, and partners. Includes solution mapping, stakeholder map, and execution plan. Use after customer agrees to proceed.',
    ...
    },
    {
      ...
      icon: '🏆',
      description: `Why Microsoft — a customer-shareable brief mapping ${companyName}'s priorities to Microsoft's 5 key differentiators. Includes objection-handling talk tracks and public comparison references.`,
      prompt: `${fullContext}\n\nTASK: Create a competitive differentiation brief for ${companyName}. Title: "${companyName} — Why Microsoft 365 Copilot: A Differentiation Brief".\n\nThis is a CUSTOMER-SHAREABLE document. It helps the AE articulate why Microsoft's AI platform is uniquely positioned to deliver on ${companyName}'s priorities — grounded in facts, not competitive attacks.\n\nIMPORTANT RULES:\n- This document IS shareable with the customer\n- Ground every claim in ${companyName}'s specific priorities, challenges, and use cases from the context above\n- Use the 5 Microsoft differentiation pillars below as the structure\n- Include talk tracks for common objection scenarios\n- NEVER make negative claims about any other vendor — focus on Microsoft's value on its own merits\n- All comparison framing should be "what Microsoft offers" not "what others lack"\n\nSTRUCTURE:\n\n1. EXECUTIVE SUMMARY (half page)\n- Opening: "${companyName}'s priorities in ${industryName} require an AI platform that [connects their specific needs to platform requirements]"\n- One paragraph connecting their stated priorities to the need for a comprehensive, secure, enterprise-grade AI platform\n- Key message: AI is not a standalone tool — it must be embedded in how your people already work\n\n2. YOUR PRIORITIES, OUR PLATFORM (1 page)\n- Create a table mapping each of ${companyName}'s stated priorities to specific Microsoft capabilities\n- For each priority: what they need → what Microsoft delivers → expected outcome\n- Reference specific use cases from the Frontier pillar analysis above\n\n3. FIVE REASONS MICROSOFT COPILOT STANDS APART\n\n3a. WORK IQ — AI That Knows Your Business\n- Microsoft 365 Copilot is grounded in Work IQ — it understands organizational context from meetings, emails, chats, documents, and people relationships\n- Unlike standalone AI chat tools, Copilot reasons over your company's actual work context — not just what you paste into a prompt\n- For ${companyName}: explain how Work IQ applies to their specific challenges (e.g., "When your [role] asks Copilot about [priority], it already understands the context from recent meetings, project documents, and team conversations")\n\n3b. AGENTIC WORK — From Answers to Action\n- Copilot doesn't just answer questions — it takes action across Microsoft 365 apps, creating real outputs (documents, presentations, emails, data analysis)\n- Copilot Cowork enables long-running, multi-step work: it breaks down complex requests, reasons across tools and files, and delivers observable progress you can review and guide\n- For ${companyName}: map this to their specific use cases (e.g., "For [use case], Copilot can [specific action] directly in [app], saving [time/effort]")\n\n3c. ENTERPRISE AGENT PLATFORM — Build, Deploy, Govern\n- A built-in platform for anyone to use pre-built agents or build custom ones — from simple task agents to advanced multi-step workflow agents\n- Agent 365 provides centralized governance, lifecycle management, and policy enforcement for all agents\n- Copilot Studio enables no-code to pro-code agent building within one governed platform\n- For ${companyName}: suggest 2-3 agent scenarios specific to their industry and challenges\n\n3d. SECURITY BY DESIGN — Not Bolted On\n- Copilot inherits your existing Microsoft 365 security, permissions, data loss prevention, auditing, and compliance controls — automatically\n- Enterprise Data Protection (EDP) applies to all data Copilot reasons over\n- Users only see information they're authorized to access — no additional configuration needed\n- For ${companyName}: reference their industry's compliance requirements (${industryName} regulations, data residency, etc.)\n\n3e. MULTI-MODEL CHOICE — Right Model for Each Task\n- Access multiple leading AI models within a single secure platform — no tool-switching required\n- The right model is selected per task type, balancing capability, speed, and cost\n- For ${companyName}: explain how different models serve different use cases in their context\n\n4. ADDRESSING COMMON QUESTIONS (1 page)\nCreate a table with 4-5 common objection scenarios and responses. Adapt these to ${companyName}'s context:\n\n| "We already have..." | Why Microsoft Is Different |\n|---|---|\n| An AI assistant / chat tool | "Copilot isn't a separate tool — it's embedded in the apps your people already use every day. Work IQ means it understands your company's context, not just what you type. The result: AI that works in your flow, not a new workflow to learn." |\n| A productivity suite with some AI features | "The difference is depth. Copilot reasons across your entire Microsoft 365 environment — emails, meetings, documents, people — to deliver answers grounded in your actual business context. Standalone AI features can't connect those dots." |\n| An AI platform investment underway | "Copilot complements existing investments. It works alongside your current tools and data, and the agent platform lets you extend AI into custom workflows without replacing what's already working." |\n| Concerns about data security with AI | "Copilot doesn't train on your data. It inherits the same security and compliance controls you already trust in Microsoft 365 — permissions, DLP, auditing, data residency — all enforced automatically." |\n\nCustomize these to ${companyName}'s specific situation. If their priorities suggest they're evaluating alternatives, tailor the responses to address their particular concerns.\n\n5. WHAT THIS MEANS FOR ${companyName.toUpperCase()} (half page)\n- Synthesis paragraph: connect their top 3 priorities to the 2-3 most relevant differentiators\n- Quantify where possible using ROI ranges from the evidence above\n- End with a clear "next step" invitation\n\n6. REFERENCES\nInclude these public comparison resources as links:\n- Microsoft 365 Copilot vs. the Competition: https://www.microsoft.com/en-us/microsoft-365-copilot/copilot-vs-the-competition\n- Compare with ChatGPT Enterprise: https://www.microsoft.com/en-us/microsoft-365-copilot/copilot-vs-chatgpt-enterprise\n- Compare with Claude Enterprise: https://www.microsoft.com/en-us/microsoft-365-copilot/copilot-vs-claude-enterprise\n- Compare with Gemini Enterprise: https://www.microsoft.com/en-us/microsoft-365-copilot/copilot-vs-gemini-enterprise\n\n7. DISCLAIMERS (footer)\nInclude all standard compliance disclaimers from the guidelines above.\n\nDESIGN:\n- Professional, scannable — designed for a customer executive audience\n- Use bold headers, short bullets, white space\n- ${brandColorInstruction}\n- Tables for structured comparisons\n- Total length: 3-5 pages when printed\n\nOUTPUT: Create a single HTML file (.html) with embedded CSS — print-ready (@media print styles), professional layout. Opens in browser, prints cleanly. No external dependencies.`,
    },
  ]
