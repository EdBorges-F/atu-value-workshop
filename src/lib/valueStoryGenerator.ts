  ...
  const coworkPrompts: CoworkPrompt[] = [
    {
      ...
      icon: '🔍',
      description: `The master prompt — builds a complete use case catalog (confirmed + expansion ideas) organized by pillar. Produces HTML + Word. Start here to map the full opportunity, then use targeted prompts below.${missingPillars.length > 0 ? ` Includes ${missingPillars.length} unexplored pillar${missingPillars.length > 1 ? 's' : ''}.` : ''}`,
    ...
    },
    {
      ...
      icon: '📊',
      description: 'Present the opportunity — a 5-8 slide deck with pillars, use cases, ROI, and next steps. Produces HTML (present in browser) + PPTX. Use after Use Case Discovery to create the executive presentation.',
    ...
    },
    {
      ...
      label: 'Business Case',
      icon: '💰',
      description: 'Justify the investment — a financial business case with ROI scenarios, delay risk, and phased roadmap. Produces HTML for the CFO. Uses public financials when available, otherwise percentage-based projections.',
